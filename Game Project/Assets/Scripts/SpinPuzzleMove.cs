...
public class SpinPuzzleMove : MonoBehaviour
{
    [Header("Grid Move")]
    ...
    {
        Debug.Log("TryPushFromPlayer from Puzzle is Triggeredyy");
        if (moving) return;

        Debug.Log("After moving is checked.");

        Movement movement = playerCollider.GetComponentInParent<Movement>();
        if (movement == null) return;
        Debug.Log("successfully got player collider");
        ...
        Debug.Log("Got past the first spin time"); 
    ...
    Debug.Log("Got past spinTime returns");


    //temporairly comment this out so that I can try and fix the issue
    ...
    /*Vector3 pushDir = movement.transform.forward;
    pushDir.y = 0f;

    Vector3 dir = GetCardinalDirection(pushDir);*/
    ...
    public void TryPushFromPlayer(Collider playerCollider)
    {
        Debug.Log("ENTERED TryPushFromPlayer");

        if (moving)
        {
            Debug.Log("Blocked by moving");
            return;
        }
        ...
        if (movement == null)
        {
            Debug.Log("No Movement component found");
            return;
        }

        Debug.Log("Movement found");

        Debug.Log("IsSpinning: " + movement.IsSpinning);
        Debug.Log("LastSpinTime: " + movement.LastSpinTime);
        Debug.Log("LastConsumedSpinTime: " + lastConsumedSpinTime);

        if (!movement.IsSpinning)
        {
            Debug.Log("Blocked by NOT spinning");
            return;
        }

        float spinTime = movement.LastSpinTime;

        if (spinTime <= lastConsumedSpinTime)
        {
            Debug.Log("Blocked by already consumed spin");
            return;
        }

        Debug.Log("PASSED SPIN CHECKS");

        lastConsumedSpinTime = spinTime;

        StartCoroutine(MoveOneStep(rb.position.x + 1f, rb.position.z));
    }

    IEnumerator MoveOneStep(float targetX, float targetZ)
    {
        Debug.Log("X target is: " + targetX);
        Debug.Log("Y target is: " + targetZ);
        Debug.Log("Got to moving step");
        moving = true;
